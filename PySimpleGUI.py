    ...
    def ButtonCallBack(self):
        ...
        if self.BType == BUTTON_TYPE_BROWSE_FOLDER:
            ...
            self.TKCal = TKCalendar(master=root, firstweekday=calendar.SUNDAY, target_element=target_element,
                                    close_when_chosen=self.CalendarCloseWhenChosen)
...
def CalendarButton(button_text, target=(None, None), close_when_date_chosen=True, image_filename=None, image_data=None,
...
def PopupGetFolder(message, default_path='', no_window=False, size=(None, None), button_color=None,
    ...
    layout = [[Text(message, auto_size_text=True, text_color=text_color, background_color=background_color)],
              [InputText(default_text=default_path, size=size, change_submits=True, key=0, do_not_clear=True), FolderBrowse(initial_folder=initial_folder)],
    ...
    while True:
        (button, input_values) = window.Read()
        if button == 'OK':
            path = input_values[0]
            return path
        elif button == 0:
...
def PopupGetFile(message, default_path='', default_extension='', save_as=False, file_types=(("ALL Files", "*.*"),),
    ...
    if no_window:
        ...
        if save_as:
            filename = tk.filedialog.asksaveasfilename(filetypes=file_types,
                                                       ...
                                                       initialdir=initial_folder)  # show the 'get file' dialog box
        else:
    ...
    layout = [[Text(message, auto_size_text=True, text_color=text_color, background_color=background_color)],
              [InputText(default_text=default_path, size=size, change_submits=True, key=0, do_not_clear=True), browse_button],
